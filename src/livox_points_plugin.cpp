...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent,
  ...
  msg_type = sdf->Get<std::string>("msg_type");
  ...
  if (msg_type.compare("normal") == 0)
    rosPointPub =
        rosNode->advertise<sensor_msgs::PointCloud2>(curr_scan_topic, 5);
  else if (msg_type.compare("custom") == 0)
    rosPointPub =
        rosNode->advertise<livox_ros_driver::CustomMsg>(curr_scan_topic, 5);
  else
    throw std::invalid_argument("received invalid message type");
...
void LivoxPointsPlugin::OnNewLaserScans() {
  if (rayShape) {
    ...
    rayShape->Update();
    SendRosTf(raySensor->Pose(), world->Name(), raySensor->ParentName());

    if (msg_type.compare("normal") == 0) {
      sensor_msgs::PointCloud2 scan_point;
      getNormalMsg(points_pair, scan_point);
      rosPointPub.publish(scan_point);
    }

    if (msg_type.compare("custom") == 0) {
      livox_ros_driver::CustomMsg scan_point;
      getCustomMsg(points_pair, scan_point);
      rosPointPub.publish(scan_point);
    }
...
void LivoxPointsPlugin::getCustomMsg(
    std::vector<std::pair<int, AviaRotateInfo>> &points_pair,
    livox_ros_driver::CustomMsg &scan_point) {
  ...
  int numpoints = samplesStep / downSample;
  ...
  for (int i = 0; i < points_pair.size(); ++i) {
    ...
    if (verticle_index < verticalRayCount && horizon_index < rayCount) {
      ...
      pt.y = point.Y();
      pt.z = point.Z();
      float reflectivity = static_cast<float>(intensity);
      if (std::isnan(pt.x) || std::isnan(pt.y) || std::isnan(pt.z) ||
          std::isnan(reflectivity))
        pt.tag = 0x4;
      else
        pt.tag = 0;

      pt.offset_time = offset_time;
      offset_time += 10;

      pt.line = pair.second.line;
      scan_point.points.push_back(std::move(pt));
      pnum++;
    }
  }

  scan_point.timebase = timestamp.nsec;
  scan_point.point_num = pnum;
  scan_point.lidar_id = 0;
  scan_point.header.stamp = timestamp;
  scan_point.header.frame_id = raySensor->Name();
}

void LivoxPointsPlugin::getNormalMsg(
    std::vector<std::pair<int, AviaRotateInfo>> &points_pair,
    sensor_msgs::PointCloud2 &scan_point) {
  msgs::Set(laserMsg.mutable_time(), world->SimTime());
